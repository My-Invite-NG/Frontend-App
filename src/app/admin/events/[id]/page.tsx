  ...
  return (
    <div className="p-8 max-w-5xl mx-auto w-full">
        ...
        <ConfirmationModal
            ...
            title={actionModal.type === 'delete' ? 'Delete Event' : actionModal.type === 'approve' ? 'Approve Event' : 'Suspend Event'}
            message={
            ...
            variant={actionModal.type === 'delete' ? 'danger' : actionModal.type === 'approve' ? 'primary' : 'warning'}
            confirmLabel={actionModal.type === 'delete' ? 'Delete' : actionModal.type === 'approve' ? 'Approve' : 'Suspend'}
        />
        ...
        <header className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
            <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className={`px-3 py-1 rounded-full text-sm font-bold tracking-wide border 
            ...
            <div className="lg:col-span-2 space-y-8">
                ...
                <div className="aspect-video bg-slate-100 rounded-2xl overflow-hidden relative border border-slate-200 shadow-sm group">
                    {event.image_url ? (
                        ...
                        <div className="flex items-center justify-center h-full text-slate-400 font-medium">No Cover Image</div>
                    )}
                </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                        <h3 className="font-bold text-slate-900 text-sm uppercase tracking-wide">Tickets</h3>
                    </div>
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 text-slate-500 font-semibold border-b border-slate-200">
                            <tr>
                                <th className="px-6 py-3 text-xs uppercase tracking-wider">Type</th>
                                <th className="px-6 py-3 text-xs uppercase tracking-wider">Price</th>
                                <th className="px-6 py-3 text-xs uppercase tracking-wider">Sold</th>
                                <th className="px-6 py-3 text-xs uppercase tracking-wider">Stock</th>
                                 ...
                                 return (
                                     <tr key={ticket.id} className="hover:bg-slate-50 transition-colors group">
                                         <td className="px-6 py-4">
                                             ...
                                             <p className="text-xs text-slate-500 truncate max-w-[150px]">{ticket.description}</p>
                                         </td>
                                         <td className="px-6 py-4 font-medium text-slate-700">{formatCurrency(ticket.price)}</td>
                                         <td className="px-6 py-4 text-slate-600">{sold}</td>
                                         <td className="px-6 py-4">
                                             <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${remaining > 0 ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-red-50 text-red-700 border-red-100'}`}>
                                                 {remaining} left
                                             </span>
                                         </td>
            ...
            <div className="space-y-6">
                 ...
                 <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div className="flex items-start gap-4">
                        <div className="w-12 h-12 rounded-full bg-violet-100 flex items-center justify-center text-violet-700 font-bold text-lg border-2 border-white shadow-sm">
                            {(event.user?.name || 'U').charAt(0)}
                        </div>
                        <div>
                            <p className="font-bold text-slate-900">{event.user?.name || 'Unknown'}</p>
                 ...
                 <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 bg-slate-50 rounded-xl text-center border border-slate-100">
                            <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-1">Sold</p>
                            <p className="text-2xl font-bold text-slate-900">{event.purchased_tickets_count || 0}</p>
                        </div>
                         {/* We'd need total sales amount from backend to make this accurate per event */}
                        <div className="p-4 bg-emerald-50 rounded-xl text-center border border-emerald-100">
                            <p className="text-[10px] text-emerald-600/70 uppercase font-bold tracking-wider mb-1">Revenue</p>
                            <p className="text-xl font-bold text-emerald-700">
                                {/* Approximate or fetch real total from backend field if added */}
                                --
                            </p>
                        </div>
                    </div>
