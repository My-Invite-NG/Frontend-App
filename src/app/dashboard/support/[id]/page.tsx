...
export default function TicketDetailPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-2rem)] md:h-[calc(100vh-4rem)] bg-white dark:bg-slate-900 shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h1 className="font-bold text-slate-900 dark:text-gray-300 text-lg truncate max-w-50 sm:max-w-md">
                {ticket.subject}
              </h1>
              <span
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
        <div className="text-center">
          <span className="text-xs text-slate-400 dark:text-gray-700 bg-white border border-slate-100 dark:border-gray-100 px-3 py-1 rounded-full shadow-sm">
            Today
          </span>
        </div>
        ...
        {messages.map((msg) => (
          ...
          >
            <div
              className={`flex flex-col max-w-[85%] sm:max-w-[70%] ${msg.is_admin ? "items-start" : "items-end"}`}
            >
              <div
                className={`flex items-end gap-2 ${msg.is_admin ? "flex-row" : "flex-row-reverse"}`}
              >
                <div
              ...
              <div
                className={`text-[10px] text-slate-400 mt-1 px-11 opacity-70 ${msg.is_admin ? "text-left" : "text-right"}`}
              >
      ...
      {/* Input Area */}
      {ticket.status !== "closed" && (
        <div className="bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 p-4 shrink-0">
          <form onSubmit={handleSend} className="max-w-4xl mx-auto flex gap-3">
            ...
            <input
              type="text"
              className="flex-1 bg-transparent border-none rounded-sm px-4 py-3 outline-none focus:ring-1 focus:text-slate-800 dark:focus:text-gray-100 text-slate-800 dark:text-gray-300 placeholder:text-slate-400 dark:placeholder:text-gray-600 transition-all font-medium"
