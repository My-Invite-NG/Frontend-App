      ...
      <div className="max-w-3xl mx-auto px-4 pt-8 md:pt-12">
        ...
        {!user && (
            <div className="bg-card rounded-2xl shadow-sm border border-border p-6 md:p-8 mb-6">
                <h2 className="text-lg font-bold text-foreground mb-6">Guest Details</h2>
        ...
        <div className="bg-card rounded-2xl shadow-sm border border-border overflow-hidden mb-6">
            <div className="p-6 md:p-8">
                <div className="flex justify-between items-start mb-6">
                    <div>
                        <h2 className="text-lg font-bold text-foreground mb-1">Ticket Summary</h2>
                ...
                <div className="space-y-3 pb-6 border-b border-border">
                    {ticketDetails.length > 0 ? ticketDetails.map(ticket => (
                        <div key={ticket.id} className="flex justify-between text-sm">
                            <span className="text-muted-foreground">{ticket.title} (x{ticketCounts[ticket.id]})</span>
                            <span className="text-foreground font-medium">{formatCurrency(ticket.price * ticketCounts[ticket.id])}</span>
                        </div>
                    )) : (
                        <div className="text-sm text-destructive">No tickets selected</div>
                    )}
                </div>
                ...
                <div className="space-y-2 py-4 border-b border-border">
                   <div className="flex justify-between text-sm">
                    ...
                    <div className="flex justify-between text-sm">
                        ...
                        <span className="text-foreground font-medium">{formatCurrency(serviceFee)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
        ...
        {/* Payment Methods */}
        <div className="bg-card rounded-2xl shadow-sm border border-border p-6 md:p-8 mb-6">
            <h2 className="text-lg font-bold text-foreground mb-6">Choose Payment Method</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                ...
                >
                    ...
                    <p className="text-xs text-muted-foreground">Credit/Debit Card, Bank Transfer</p>
                </button>
                ...
                >
                    ...
                    <p className="text-xs text-muted-foreground">Credit/Debit Card, USSD, QR Code</p>
                </button>
                ...
                {/* Wallet Payment */}
                 {user && (
                    ...
                    >
                        ...
                        <p className="text-xs text-muted-foreground mb-1">Pay with your wallet balance</p>
                        <p className={`text-sm font-bold ${walletSufficient ? 'text-emerald-600' : 'text-destructive'}`}>
                            Balance: {formatCurrency(walletBalance)}
                        </p>
                        {!walletSufficient && <p className="text-[10px] text-destructive mt-1">Insufficient funds</p>}
                    </button>
                 )}
            </div>
            ...
            <button 
                ...
                className="w-full py-3.5 bg-muted hover:bg-primary hover:text-primary-foreground text-muted-foreground font-bold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed data-[state=active]:bg-primary data-[state=active]:text-primary-foreground flex justify-center items-center gap-2"
            >
                {processing ? <Loader2 className="w-5 h-5 animate-spin text-current" /> : 'Proceed to Payment'}
            </button>
        </div>
        ...
        <div className="bg-card rounded-2xl shadow-sm border border-border p-6 md:p-8">
            ...
            <div className="flex flex-col md:flex-row gap-6 mb-8">
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground mb-1">Available 24/7 for payment issues</p>
                        <a href="#" className="text-xs text-primary font-medium hover:underline">support@eventticket.com</a>
                    </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground mb-1">Find answers to common questions</p>
                        <a href="#" className="text-xs text-primary font-medium hover:underline">View payment FAQ</a>
                    </div>
                </div>
            </div>
            ...
            <div className="border-t border-border pt-6 flex flex-wrap justify-center gap-6 text-xs text-muted-foreground">
             ...
             <p className="text-[10px] text-muted-foreground text-center mt-4">
                By proceeding with the payment, you agree to our <Link href="/terms" className="text-primary hover:underline">Terms of Service</Link> and <Link href="/privacy" className="text-primary hover:underline">Privacy Policy</Link>.
             </p>
